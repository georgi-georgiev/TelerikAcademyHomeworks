...
namespace PathBetweenTwoCells
{
    class Program
    {
        static char[,] labyrint = new char[100, 100];

        static List<char> path = new List<char>();

        static bool InRange(int row, int col)
        {
            bool rowInRange = row >= 0 && row < labyrint.GetLength(0);
            bool colInRange = col >= 0 && col < labyrint.GetLength(1);
        ...
        static void Main()
        {
            ...
            {
                for (int l = 0; l < labyrint.GetLength(1); l++)
                {
                    labyrint[i, l] = ' ';
                }
            }
